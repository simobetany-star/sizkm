...
export function PDFSignatureManager({
  ...
  useEffect(() => {
    if (currentSignaturePosition) {
      const defaultPosition = { x: 100, y: 100, width: 200, height: 60, opacity: 0.7, page: 1 };
      setSignaturePosition({ ...defaultPosition, ...currentSignaturePosition });
    }
  }, [currentSignaturePosition]);

  useEffect(() => {
    if (currentDualPositions) {
      const defaultPosition = { x: 100, y: 100, width: 180, height: 50, opacity: 0.7, page: 1 };
      const safePositions = {
        staff: { ...defaultPosition, ...currentDualPositions.staff },
        client: { ...defaultPosition, ...currentDualPositions.client }
      };
      setDualPositions(safePositions);
    }
  }, [currentDualPositions]);

  const loadCurrentSignaturePosition = () => {
    // If we have current positions from props, use them
    const defaultPosition = { x: 100, y: 100, width: 180, height: 50, opacity: 0.7, page: 1 };

    if (isDualSignature && currentDualPositions) {
      const safePositions = {
        staff: { ...defaultPosition, ...currentDualPositions.staff },
        client: { ...defaultPosition, ...currentDualPositions.client }
      };
      setDualPositions(safePositions);
      return;
    } else if (!isDualSignature && currentSignaturePosition) {
      setSignaturePosition({ ...defaultPosition, ...currentSignaturePosition });
      return;
    }

    // Fallback to default positions if no current positions provided
    ...
    if (formTypeKey) {
      // Set default positions based on form type (these match our server config)
      const defaultSinglePositions: Record<string, SignaturePosition> = {
        "absa-form": { x: 74, y: 373, width: 200, height: 60, page: 1 },
        "sahl-certificate-form": { x: 71, y: 586, width: 200, height: 60, page: 1 },
        "noncompliance-form": { x: 300, y: 700, width: 200, height: 60, page: 1 },
        "material-list-form": { x: 320, y: 750, width: 200, height: 60, page: 1 }
      };

      const defaultDualPositions: Record<string, DualSignaturePositions> = {
        "clearance-certificate-form": {
          signature: { x: 87, y: 575, width: 180, height: 50, page: 1 },
          signature_staff: { x: 87, y: 631, width: 180, height: 50, page: 1 }
        },
        "discovery-form": {
          signature: { x: 120, y: 163, width: 180, height: 50, page: 1 },
          signature_staff: { x: 120, y: 110, width: 180, height: 50, page: 1 }
        },
        "liability-form": {
          signature: { x: 411, y: 788, width: 180, height: 50, page: 1 },
          signature_staff: { x: 4, y: 788, width: 180, height: 50, page: 1 }
        }
      };

      if (isDualSignature && defaultDualPositions[formTypeKey]) {
        setDualPositions(defaultDualPositions[formTypeKey]);
      } else if (!isDualSignature && defaultSinglePositions[formTypeKey]) {
        setSignaturePosition(defaultSinglePositions[formTypeKey]);
      }
    }
  };
  ...
  const getCurrentPosition = (): SignaturePosition => {
    ...
    if (isDualSignature) {
      const pos = dualPositions[activeSignature];
      return pos ? { ...defaultPosition, ...pos } : defaultPosition;
    }
    return signaturePosition ? { ...defaultPosition, ...signaturePosition } : defaultPosition;
  };
              ...
              <CardContent className="space-y-4">
                ...
                <div className={`p-3 rounded-lg border-2 transition-all duration-200 ${
                  ...
                  <div className="text-lg font-mono mt-1">
                    X: <span className="text-blue-600 font-bold">{getCurrentPosition().x || 0}</span>,
                    Y: <span className="text-blue-600 font-bold">{getCurrentPosition().y || 0}</span>
                  </div>
                  <div className="text-sm text-gray-600 mt-1">
                    Size: {getCurrentPosition().width || 180} × {getCurrentPosition().height || 50}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <Input
                      id="sig-width"
                      type="number"
                      value={getCurrentPosition().width || 180}
                      onChange={(e) => updateCurrentPosition({
                  ...
                  <div>
                    ...
                    <Input
                      id="sig-height"
                      type="number"
                      value={getCurrentPosition().height || 50}
                      onChange={(e) => updateCurrentPosition({
