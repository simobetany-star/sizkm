...
export function StaffProfileModal({
  ...
  const [godFlag, setGodFlag] = useState<boolean | undefined>(staffMember?.god_staff);
  ...
  useEffect(() => {
    if (open && staffMember) {
      ...
      fetchJobPhotos();
      fetchUsers();
      setGodFlag(staffMember.god_staff);
    }
  }, [open, staffMember]);
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      // If admin toggled god flag, persist it via user update endpoint
      if ((user?.role === 'admin' || user?.role === 'super_admin') && typeof godFlag !== 'undefined' && godFlag !== staffMember.god_staff) {
        const userUpdateHeaders = { ...headers, "Content-Type": "application/json" };
        const userUpdateRes = await fetch(`/api/auth/users/${staffMember.id}`, {
          method: 'PUT',
          headers: userUpdateHeaders,
          body: JSON.stringify({ god_staff: !!godFlag })
        });
        if (!userUpdateRes.ok) {
          const t = await userUpdateRes.text().catch(() => '');
          throw new Error(t || 'Failed to update God Staff flag');
        }
      }
                      ...
                      <div className="mt-6 p-3 border rounded-md">
                        ...
                        <label className="flex items-center gap-2 text-sm">
                          <input
                            type="checkbox"
                            checked={!!godFlag}
                            onChange={(e) => setGodFlag(e.target.checked)}
                          />
                          <span>Mark as Vinesh God staff</span>
                        </label>
                        <p className="text-xs text-gray-500 mt-1">God staff are treated as existing Vinesh company staff and are excluded from Typo Work.</p>
                      </div>
                    )}
                  </div>
                )}
