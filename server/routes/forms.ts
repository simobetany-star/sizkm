...
export const handleGetFormSubmissions: RequestHandler = async (req: any, res) => {
  try {
    ...
    // Apply role-based data filtering for security
    const { filterFormSubmissionsForUser, logDataAccess } = await import('../utils/dataFilter');
    const { jobs } = await import('./jobs');

    const secureSubmissions = filterFormSubmissionsForUser(filteredSubmissions, jobs, user?.role || 'staff');

    // Log data access for security monitoring
    logDataAccess(user?.id || 'anonymous', user?.role || 'unknown', 'form-submissions', secureSubmissions.length);

    res.json(secureSubmissions);
  } catch (error) {
