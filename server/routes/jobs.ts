...
export const handleGetJobs: RequestHandler = async (req: any, res) => {
  try {
    ...
    const { filterJobsForUser, logDataAccess } = await import('../utils/dataFilter');
    const secureJobs = filterJobsForUser(filteredJobs, user?.role || 'staff');

    // Log data access for security monitoring
    logDataAccess(user?.id || 'anonymous', user?.role || 'unknown', 'jobs', secureJobs.length);
