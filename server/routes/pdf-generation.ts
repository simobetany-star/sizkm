...
// Overlay company branding (name/logo placeholder) on first page
async function overlayBranding(pdfDoc: PDFDocument, jobId?: string) {
  try {
    if (!jobId) return;
    ...
    const name: string | undefined = company?.name;
    const page = pdfDoc.getPages()[0];
    if (page && name) {
      page.drawText(name, { x: 20, y: page.getHeight() - 20, size: 10, color: rgb(0, 0, 0) });
    }
  } catch (e) {
...
async function generateABSAPDF(submission: any): Promise<Uint8Array> {
  ...
  await overlayBranding(pdfDoc, submission.jobId);
...
export const handleGenerateMaterialListPDF: RequestHandler = async (
  ...
  try {
    ...
    await overlayBranding(pdfDoc, (materialListData as any).jobId);
